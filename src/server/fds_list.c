...
FDS_LIST* fds_list_init() {
    ...
    fds_list->fd_timestamps = malloc(sizeof(time_t) * MAX_SOCKETS_COUNT);
    fds_list->size = 0;
    return fds_list;
}
...
void fds_list_insert(FDS_LIST* fds_list, struct pollfd fd) {
    ...
    fds_list->fd_timestamps[fds_list->size] = time(NULL);
    fds_list->size++;
...
void fds_purge(FDS_LIST* fds_list) {
    for (int i = 0; i < fds_list->size; i++) {
        if (fds_list->array[i].fd == -1) {
            ...
            fds_list->fd_timestamps[i] = fds_list->fd_timestamps[fds_list->size - 1];
